<template>
  ...
  >
    ...
    <div class="portal-home__header" :style="{ paddingTop: menuButtonInfo.bottom + 'px' }">
      ...
      <div v-if="showSettlementReminder" class="portal-home__settlement-reminder">
        <span class="portal-home__settlement-icon" aria-hidden="true">
          <span>￥</span>
        </span>
        <scroll-view
          class="portal-home__settlement-copy-scroll"
          :scroll-x="true"
          :enhanced="true"
          :bounces="false"
          :show-scrollbar="false"
        >
          <p class="portal-home__settlement-copy">
            <span>您有</span><strong>费用尚未结算</strong><span>，请尽快处理</span>
          </p>
        </scroll-view>
        <button
          class="portal-home__settlement-action"
          type="button"
          hover-class="is-active"
          @tap.stop="handleSettlementReminderAction"
        >立即结算</button>
        <button
          class="portal-home__settlement-close"
          type="button"
          aria-label="关闭结算提醒"
          hover-class="is-active"
          @tap.stop="handleSettlementReminderClose"
        >
          <span class="portal-home__settlement-close-line is-forward" aria-hidden="true"></span>
          <span class="portal-home__settlement-close-line is-backward" aria-hidden="true"></span>
        </button>
      </div>
    </div>
...
const showSettlementReminder = ref(true)
...
const handleSettlementReminderAction = () => {
  Taro.navigateTo({
    url: '/pages/common-list/settlement-payment/index',
    fail(err) {
      console.error('Navigate to settlement payment page failed', err)
    }
  })
}

const handleSettlementReminderClose = () => {
  showSettlementReminder.value = false
}
...
const handleSpecialtyBannerClick = (index: number) => {
  if (index === 0) {
    Taro.navigateTo({
      url: '/pages/common-list/article-detail/index?id=3',
      fail(err) {
        console.error('Navigate to Moxa article failed:', err)
      }
    })
  } else {
    Taro.navigateTo({
