...
## Figma 还原度约束规则 (figma-implement-design)

在后续的所有开发任务中，一旦涉及到 Figma 设计稿实现或前端 UI 代码生成，必须每次都自动启用并遵守 `figma-implement-design` 技能和规范：
1. **Figma MCP 取数**：不靠截图猜尺寸，直接从 Figma 节点中精准读取布局、颜色、字体、间距和圆角。
2. **1:1 视觉还原**：代码实现结果尽量贴近设计稿，达到像素级还原。
3. **Design Tokens**：优先使用项目设计系统里的颜色、字号和组件变量（如 Less/CSS 变量）。
4. **Assets 导出**：图标、图片、SVG 等资源按 Figma 来源高清处理（优先使用 SVG 格式）。
5. **项目约定**：遵循现有前端工程结构和组件规范。
6. **实现检查**：通过对比最终代码效果截图与 Figma 设计稿，主动发现并自我修正视觉偏差。
